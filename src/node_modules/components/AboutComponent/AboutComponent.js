...
export const AboutComponent = ({header, mainText, subText, buttonText, buttonLink, image, backgroundColor}) => {
  return (
    ...
    {header ? <div className="header-component">
        <h3><FontAwesomeIcon className="icon" icon={faInfoCircle} /> {header}</h3>
    </div> : null}
          ...
          <Row>
            <Col md={6} className="left-col">
              <div className="about-content-section">
                {mainText ? <h1 className="about-h1">
                  <span className='colored-text'>{mainText.split(' ')[0]}</span>
                  <span>{mainText.split(' ').map((t, i) => i !== 0 ? ' ' + t : null)}</span>
                </h1> : null}
                <p className="about-para">{subText}</p>
                {buttonText ? <a href={buttonLink} className="btn">
                    <span className="about-span">{buttonText}</span>
                </a> : null}
              </div>
            </Col>
